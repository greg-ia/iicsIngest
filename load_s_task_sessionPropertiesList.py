...
class TaskSessionPropertiesProcessor:
    ...
    def clear_table(self) -> bool:
        ...
        # Usar TRUNCATE para limpar a tabela (mais rápido e reseta auto_increment)
        ...
        try:
            with self.db_manager.get_cursor() as cursor:
                cursor.execute(query, (
                    ...
                    property_data['dt_inserted'],  # Usando o Dt_Inserted da task
    ...
    def process_task(self, task: Dict) -> Tuple[int, int]:
        ...
        task_dt_inserted = task['Dt_Inserted']  # Data de inserção original da task
        ...
        if total > 0:
            print(f"   📊 {task_name}: {inseridas}/{total} properties (Dt_Inserted: {task_dt_inserted})")
