#!/usr/bin/env python3
"""
Script para extrair arquivos @3.bin de arquivos .DTEMPLATE.zip e converter para JSON
Uso exclusivo para extração de binários do IICS
"""
...
class BinaryExtractor:
    """Extrator de arquivos @3.bin de arquivos .DTEMPLATE.zip"""
    ...
    def load_config(self):
        ...
        # Tenta primeiro a variável de ambiente específica, senão usa o padrão
        ...
        required_keys = ['json_file_path', 'PathMapsGrava']
        missing_keys = [key for key in required_keys if key not in self.config]
        
        # Para compatibilidade com versões anteriores, verifica também outras chaves possíveis
        if 'PathMapsGrava' not in self.config and 'PathMapsGrava' in self.config:
        ...
        if missing_keys:
            ...
            print(f"       Chaves esperadas: json_file_path, PathMapsGrava")
            sys.exit(1)
    ...
    def extrair_arquivo_bin(self, arquivo_zip, caminho_zip, caminho_destino):
        """
        Extrai e renomeia o arquivo @3.bin de um único arquivo ZIP
    ...
    def processar_pasta(self, nme_projeto_iics, pasta):
        ...
        path_maps_grava = self.config.get("PathMapsGrava")
        ...
        print(f"📂 Destino: {path_maps_grava}")
        ...
        for idx, arquivo_zip in enumerate(sorted(arquivos_zip), 1):
            sucesso, mensagem = self.extrair_arquivo_bin(
                arquivo_zip, 
                caminho_origem, 
                path_maps_grava
            )
    ...
    def extrair_binario(self, projetos_info):
        """
        Função principal para extração de binários
        
        Args:
            projetos_info (list): Lista de dicionários com informações dos projetos
        """
        print("\n" + "="*60)
        print("🚀 EXTRATOR DE BINÁRIOS IICS (@3.bin → .json)")
        print("="*60)
        
        # Garantir que o diretório de destino existe
        path_maps_grava = self.config.get("PathMapsGrava")
        if not self.criar_diretorio_seguro(path_maps_grava):
        ...
        print(f"📁 Diretório destino: {path_maps_grava}")
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("  2. Extrair arquivos @3.bin de cada .DTEMPLATE.zip")
        print("  3. Converter para arquivos .json")
        print("="*60)
        sys.exit(1)
    ...
    exit_code = extractor.extrair_binario(projetos_info)
